...
(function($) {
						...
						if(s.get(0)
							...
							if(n.changeInput) {
								switch(typeof n.changeInput) {
									case "boolean":
										o = $('<div class="jFiler-input"><div class="jFiler-input-button">' + n.captions.button + '</div></div>"');
										break;
									case "string":
									case "object":
										o = $(n.changeInput);
					...
					_thumbCreator: {
						...
						renderFile: function(file, html, opts) {
							...
							if(file.file && opts.type == "image") {
								var g = '<img class="zoomify" src="' + file.file + '" draggable="false" />',
							...
							if(window.File && window.FileList && window.FileReader && opts.type == "image" && opts.size < 6e+6) {
								...
								y.onload = function(e) {
									var g = '<img class="zoomify" src="' + e.target.result + '" draggable="false" />',
										m = html.find('.jFiler-item-thumb-image.fi-loading');
									$(g)
										.error(function() {
											g = f._thumbCreator.generateIcon(opts);
	...
	$.fn.filer.defaults = {
		...
		options: null,
		captions: {
			button: "Upload Related Document Images",
